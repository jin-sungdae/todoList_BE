...
import com.example.save42lms.persistence.TodoRepository;
...
public class TodoService {
    ...
    public List<TodoEntity> update(final TodoEntity entity){
        ...
        original.ifPresent(todo-> {
            ...
            todo.setTodoCheck(entity.isTodoCheck());
    ...
    public List<TodoEntity> delete(final TodoEntity entity){
        // (1) 저장할 엔티티가 유효한지 확인한다. 이 메서드는 2.3.1 Create Todo 에서 구현함
        validate(entity);
        ...
        try{
            ...
            log.error("error deleting entity Todo", entity.getId(), e);

            // (4) 컨트롤러 exception을 보낸다. 데이터베이스 내부 로직을 캡슐화 하려면 e를 리턴하지 않고 새
            // exception 오브젝트를 리턴한다.
            throw new RuntimeException("error deleting entity Todo " + entity.getId());
        }
